...
class TokenManager: NSObject {
    ...
    func initializeToken() { //call this in app delegate
        if let accessToken =  UserDefaults.standard.string(forKey: NetworkKeys.UserDefaults.accessToken) {//, let refreshToken =  UserDefaults.standard.string(forKey: NetworkKeys.UserDefaults.refreshToken) {
